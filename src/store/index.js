...
export default new Vuex.Store({
  ...
  actions: {
    signUp(context, payload) {
      ...
      axios({
        method: 'post',
        url: `http://3.112.101.89/accounts/signup/`,
        data: {
    ...
    userData(context) {
      axios({
        method: 'get',
        url: `http://3.112.101.89/accounts/user_detail/`,
        headers: {
    ...
    savePlayListDetail(context, list_id) {
      axios({
        method: 'get',
        url: `http://3.112.101.89/movies/playlist_detail/${list_id}/`,
        headers: {
          Authorization: `Token ${context.state.token}`
        }
      })
    ...
    getRankCommentList(context) {
      axios({
        method: 'get',
        url: 'http://3.112.101.89/worlds/rankcomment_list/',
        headers: {
          Authorization: `Token ${context.state.token}`
        }
      })
