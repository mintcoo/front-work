<template>
    ...
    <div id='game'>
      ...
      <div id='ask' style="display:none">
        <p>{{ ask }}</p>
        <button style="margin-right: 3px" @click='openThirdButton'>예</button>
        <button style="margin-left: 3px" @click='sendResult'>대전 종료</button>
      </div>
      <p>{{ battle_result }}</p>
      <button id='endgame' style="display: none" @click='sendResult'>대전 종료</button>
      <button id='renewpage' style="display: none" @click='renewPage'>매칭 페이지로 돌아가기</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    sendResult() {
      axios({
        method: 'post',
        url: 'http://3.112.101.89/worlds/make_battlelog/',
        data: {
      ...
      .then((res) => {
        ...
        axios({
          method: 'post',
          url: 'http://3.112.101.89/worlds/change_tier/',
          data: {
            'after': after_tier,
          },
          headers: {
    ...
    getEnermy() {
      ...
      axios({
        method: 'get',
        url: 'http://3.112.101.89/worlds/get_enermy_status/',
        headers: {
          Authorization: `Token ${this.$store.state.token}`
        }
      })
  ...
  created() {
    ...
    axios({
      method: 'get',
      url: 'http://3.112.101.89/worlds/get_my_status/',
      headers: {
        Authorization: `Token ${this.$store.state.token}`
      }
    })
